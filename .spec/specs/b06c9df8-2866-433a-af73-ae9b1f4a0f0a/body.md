...
Specification for replacing ad-hoc `web_sys::console::log_1!()` calls in the Dioxus WASM frontend (`tools/viewer/viewer-api/frontend/dioxus`, shared by `doc-viewer`, `log-viewer`, `ticket-viewer`, `spec-viewer`) with structured `tracing` instrumentation.
