...
Canonical specification for `viewer-api::source` — the safe source-file serving helper that resolves a workspace-relative path against a configured root, rejects path traversal, and streams text files with language hinting.
