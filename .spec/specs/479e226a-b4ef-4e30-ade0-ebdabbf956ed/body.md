...
Specification for shipping `tracing` records emitted by the Dioxus WASM frontend (see [`viewer-api/tracing`](../tracing)) to a server-side endpoint that writes them to a file on disk.
