...
Canonical specification for `viewer-api::session` — the lightweight server session helper (cookie-backed UUID + per-session key/value store) used by viewers to persist non-sensitive user state across reloads.
