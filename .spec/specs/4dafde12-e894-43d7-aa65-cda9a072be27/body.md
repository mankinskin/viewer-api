...
The viewer-ctl command-line interface is defined by `clap` in `src/cli.rs`. This document is the contract: **the surface defined here is stable** and must not be broken without bumping the binary's compatibility expectations.
