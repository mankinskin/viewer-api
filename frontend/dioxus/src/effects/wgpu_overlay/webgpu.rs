...
pub(super) fn create_shader(
    ...
    get_fn(device, "createShaderModule")?
        .call1(device, &desc.into())
        .ok()
}
...
pub(super) fn create_pipeline_layout(
    ...
    get_fn(device, "createPipelineLayout")?
        .call1(device, &desc.into())
        .ok()
}
...
pub(super) fn create_compute_pipeline(
    ...
    get_fn(device, "createComputePipeline")?
        .call1(device, &desc.into())
        .ok()
}
...
pub(super) fn create_render_pipeline(
    ...
    get_fn(device, "createRenderPipeline")?
        .call1(device, &desc.into())
        .ok()
}
...
pub(super) fn create_depth_texture(
    ...
    let tex = get_fn(device, "createTexture")?
        .call1(device, &desc.into())
        .ok()?;
