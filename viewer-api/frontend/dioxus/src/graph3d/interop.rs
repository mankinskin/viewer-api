...
pub(crate) fn js_f64(v: f64) -> JsValue {
    JsValue::from_f64(v)
}
...
pub(crate) fn js_call(
    ...
    match args.len() {
        ...
        1 => {
            f.call1(target, args[0]).ok();
        },
        2 => {
            f.call2(target, args[0], args[1]).ok();
        },
        3 => {
            f.call3(target, args[0], args[1], args[2]).ok();
        },
        _ => {},
    }
}
