...
fn build_node_quad_pipeline(
    ...
    let q_attrs = Array::new();
    q_attrs.push(&JsValue::from(q_attr));
    let q_layout = obj();
    ...
    let i_attrs = Array::new();
    i_attrs.push(&JsValue::from(i_attr));
    let i_layout = obj();
    ...
    let targets = Array::new();
    targets.push(&JsValue::from(target));
