...
impl GpuBuffers {
    ...
    pub fn new(
        ...
        let elem_buf = gpu_buffer(
            device,
            (INITIAL_ELEM_CAP * ELEM_BYTES) as u32,
            USAGE_STORAGE | USAGE_COPY_DST,
        )?;
        let particle_buf = gpu_buffer(
            device,
            PARTICLE_BUF_SIZE as u32,
            USAGE_STORAGE | USAGE_COPY_DST,
        )?;
        let palette_buf = gpu_buffer(
            device,
            PALETTE_BYTE_SIZE as u32,
            USAGE_UNIFORM | USAGE_COPY_DST,
        )?;
